...
tweedie <- function(lambda, alpha, beta){
  ...
  m <- round(mean(S_N), 2)
  sd <- round(sd(S_N), 2)
  print(hist(S_N, main = "Distribution Plot for Tweedie Distribution", xlab = "Aggregate losses"))
  return(c(m,sd))
}
...
```
 ...
 The simualted mean aggregate loss is \$`r value[1]` with a standard deviation of \$`r value[2]`. This compares favorably with theory based methods as we would estimate our mean aggregate loss to be
